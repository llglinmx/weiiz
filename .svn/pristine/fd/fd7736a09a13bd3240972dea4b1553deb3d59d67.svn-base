...
function calLegendData (series, opts, config, chartData) {
  ...
  // 新增legend.format.format配置
  const formatLegend = typeof opts.legend.format === 'function' ? opts.legend.format : item => item.name;
  series.forEach((item, index) => {
    item.legendName = formatLegend(item, index);
  });
  ...
  if (opts.legend.position == 'top' || opts.legend.position == 'bottom') {
    ...
    for (let i = 0; i < series.length; i++) {
      const item = series[i];
      const itemWidth = shapeWidth + shapeRight + measureText(item.legendName || 'undefined', fontSize) + opts.legend.itemGap;
    ...
    if (currentRow.length) {
      for (let i = 0; i < currentRow.length; i++) {
        ...
        let maxWidth = 0;
        for (let j = 0; j < item.length; j++) {
          const itemWidth = shapeWidth + shapeRight + measureText(item[j].legendName || 'undefined', fontSize) + opts.legend.itemGap;
...
function getPieTextMaxLength (series) {
  ...
  for (let i = 0; i < series.length; i++) {
    const item = series[i];
    const text = item.format ? item.format(+item._proportion_.toFixed(2), item) : util.toFixed(item._proportion_ * 100) + '%';
...
function calYAxisData (series, opts, config) {
  ...
  if (YLength > 0) {
    ...
    for (let i = 0; i < YLength; i++) {
      ...
      const yAxisFontSizes = yData.fontSize || config.fontSize;
      ...
      const calibration = yData.calibration ? 4 * opts.pixelRatio : 0;
...
function drawPieText (series, opts, config, context, radius, center) {
  ...
  for (let i = 0; i < seriesConvert.length; i++) {
    ...
    const textWidth = measureText(item.text, item.textSize || config.fontSize);
    let startY = orginY3;
...
function drawToolTipHorizentalLine (opts, config, context, eachSpacing, xAxisPoints) {
  ...
  if (toolTipOption.yAxisLabel) {
    ...
    for (let i = 0; i < labelText.length; i++) {
      context.setFontSize(config.fontSize);
      const textWidth = measureText(labelText[i], config.fontSize);
...
function drawYAxis (series, opts, config, context) {
  ...
  for (let i = 0; i < opts.yAxis.data.length; i++) {
    ...
    if (yData.disabled !== true) {
      ...
      const yAxisFontSize = yData.fontSize || config.fontSize;
...
function drawLegend (series, opts, config, context, chartData) {
  ...
  legendList.forEach(function (itemList, listIndex) {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      context.fillText(item.legendName, startX, startY + fontTrans);
      context.closePath();
      context.stroke();
      if (opts.legend.position == 'top' || opts.legend.position == 'bottom') {
        startX += measureText(item.legendName, fontSize) + itemGap;
        item.area[2] = startX;
      } else {
        item.area[2] = startX + measureText(item.legendName, fontSize) + itemGap;;
        startX -= shapeWidth + shapeRight;
        startY += lineHeight;
      }
    }
...
function drawCharts (type, opts, config, context) {
  ...
  if (opts.type === 'line' || opts.type === 'column' || opts.type === 'area' || opts.type === 'mix' || opts.type === 'candle') {
    ...
    let rightIndex = 0; let leftIndex = 0;
    // 计算主绘图区域左右位置
  ...
  if (opts.categories && opts.categories.length) {
    ...
    if (opts.type === 'line' || opts.type === 'area' || opts.type === 'points') {
      ...
      const _calCategoriesData = calCategoriesData(categories, opts, config, opts.chartData.xAxisData.eachSpacing);
      const xAxisHeight = _calCategoriesData.xAxisHeight;
      const angle = _calCategoriesData.angle;
      config.xAxisHeight = xAxisHeight;
      config._xAxisTextAngle_ = angle;
