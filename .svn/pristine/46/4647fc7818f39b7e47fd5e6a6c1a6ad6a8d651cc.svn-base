<template>
  <div class="s-pull-scroll" :class="customClass" :style="{height}">
    <scroll-view
    ...
    >
      <div :style="{'transform': translateY, 'transition': transition}">
        <div
        ...
        >
          <div
            ...
            :style="{color:$sTheme(loadingIconColor),'transform':downRotate}"
          ></div>
          <div>{{downText}}</div>
        </div>
        ...
        <div class="s-pull-up-wrap" v-if="enablePullUp && !isEmpty">
          <slot name="up-loading" v-if="isUpLoading">
            <div class="s-pull-loading-icon s-pull-loading-rotate" :style="{color:$sTheme(loadingIconColor)}"></div>
            <div>{{upLoadingText}}</div>
          </slot>
          <slot name="up-error" v-if="isUpError && showUpError">
            <div v-if="upErrorText" @click="onUpErrorClick">{{upErrorText}}</div>
          </slot>
          <slot name="up-finish" v-else-if="isUpFinish && showUpFinish">
            <div v-if="upFinishText">{{upFinishText}}</div>
          </slot>
        </div>
      </div>
    </scroll-view>
    <!-- 回到顶部按钮 -->
    <div class="s-pull-back-top" v-if="backTop" :class="{'is-show':isShowBackTop}" @click="onBackTop">
      <slot name="backtop">
  ...
  methods: {
    ...
    triggerPullDown () {
      if (this.pullDown && this.enablePullDown && !this.isDownLoading && !this.isUpLoading) {
        ...
        this.page = 1; // 预先加一页
        this.pullType = 'down';
        this.pullDown && getPropsFnByParents(this, this.pullDown)(this);
      }
    },
    /* 触发上拉加载 */
    triggerPullUp () {
      if (this.pullUp && this.enablePullUp && !this.isUpLoading && !this.isDownLoading && !this.isUpError && !this.isUpFinish) {
        ...
        this.showUpLoading();
        this.pullType = 'up';
        this.pullUp && getPropsFnByParents(this, this.pullUp)(this);
      }
    },
    refresh () {
      this.scrollTo(0);
...
.s-pull-scroll {
  ...
  .s-pull-loading-rotate {
    animation: s-animate-rotate 0.6s linear infinite;
  }
