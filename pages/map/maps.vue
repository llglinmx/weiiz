<template>
	<view class="map-box">
		...
		<view class="map-content" id="map-content">
			<map :style="{height:mapHeight+'rpx'}" style="width: 100%;" :enable-zoom="true" :scale="10" :enable-scroll="true" :latitude="latitude" :longitude="longitude"
				:markers="marker"></map>
					...
					if (res.status == 200) {
						if (res.data.storeList.length != 0) {
							...
							res.data.storeList.forEach(item => {
								var str = {
									id: item.id*1,
									latitude: item.latitude_baidu,
									longitude: item.longitude_baidu,
